...
4. Submit one complete `submit_relationships` attempt after this stage's
   decision is ready. Retry only after an explicit server rejection.
...
`submit_relationships` returns. A rejected submission may be corrected and
