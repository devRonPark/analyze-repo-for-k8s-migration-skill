...
`검색` marker는 번역하거나 표기를 바꾸지 않는다. `미확인`은 file:line 하나만으로 종료할 수 없고, `상충됨`은 두 참조를 `, `로 나열한다.
근거의 경로는 저장소 루트 기준 상대 경로와 줄 번호로 쓰고(`src/main/webapp/WEB-INF/applicationContext.xml:31-34`), 참조 뒤에 설명을 붙이지 않는다.
