...
| VS-021 — reject `recommendation` in Summary | DONE | `report_contract.validate_json_payload` now rejects a `recommendation`-classified `missing_inputs` entry when `mode == "summary"`; Detailed mode unaffected (dedicated test). | VS-022 (engine-enforced JSON) remains open. |
