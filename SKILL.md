...
Place every discovered item into exactly one outcome:

1. Deployable candidate
2. Repository-defined runtime dependency
3. External runtime dependency
4. Excluded from deployable candidates

A package manifest, dependency declaration, build script, CI job, Dockerfile,
Compose service, or Kubernetes manifest is evidence, not a deployable conclusion.

Classify an item as a deployable candidate only when repository evidence shows
independently executable runtime behavior.

For every excluded item, record the reason and evidence. Common exclusions include:
