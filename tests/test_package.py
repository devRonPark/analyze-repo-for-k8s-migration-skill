...
# Keep the current Summary fixture as the single contract fixture. The legacy
# string above remains only for explicit --legacy regression coverage.
NEW_VALID_SUMMARY = (ROOT / "tests/fixtures/reports/valid-summary.md").read_text(encoding="utf-8")
...
class SkillPackageTests(unittest.TestCase):
    ...
    def test_deployment_core_dependency_and_readiness_contract(self):
        text = "\n".join(
            ...
            for path in [
                ...
                ROOT / "references/dependency-analysis.md",
                ROOT / "assets/migration-summary-template.md",
                ROOT / "assets/migration-assessment-template.md",
            ]
        )
    ...
    def test_summary_rejects_detailed_only_sections(self):
        ...
        self.assertIn("Detailed 전용", result.stdout)
