...
immediately. Route to the language, configuration-timing, and dependency
references only after the corresponding finding exists.
[workload-boundary](workload-boundary.md) is not one of those conditional
references: it is always read, and its primary rule decides every candidate
boundary before `components` is counted. For Detailed, complete the full
