...
## Command Stage Separation

Record these stages separately for every deployable candidate:

1. Dependency installation command
2. Application build command
3. Image build command
4. Production startup command

Do not use one command as evidence for another stage.

Examples:

- `npm install` is not an application build command.
- `npm run build` is not a production startup command.
- `docker build` is not an application build command.
- `mvn package` is not a production startup command.
- a development server is not production startup evidence.
...
Determine the package manager per component in this order:

1. the component's packageManager declaration
2. the applicable workspace declaration
3. component-specific scripts and documentation
4. lockfile evidence

A repository-root declaration does not override stronger component-level evidence.

When package-manager evidence conflicts, report it as Conflicting or Unknown
...
When Maven and Gradle files both exist, determine their applicable module scopes.
Do not select one toolchain merely because both files are present.

Separate dependency resolution, application packaging, image creation, and
runtime startup.

Do not infer the production startup command from a build command alone.
