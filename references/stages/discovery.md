...
`locate_evidence`, or `list_target_paths` only if a current decision remains
blocked after grounding from the survey. On `precision_budget_exhausted`, stop
searching and submit `unknown`/`inferred` for the blocked candidate instead.
