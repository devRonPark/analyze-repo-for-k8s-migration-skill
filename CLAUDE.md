...
이 프로젝트의 제품은 Local Git Repository를 읽기 전용으로 탐색해 Kubernetes
Migration Plan과 manifest 초안을 생성하는 Google ADK MVP다. `AGENTS.md`와
`CONTEXT.md`가 제품 기준이다.
...
- 제품 package(`migration_assistant/`)가 개발 전용 `devtools/`를 import하는지,
  제거된 OpenCode Skill 자산이 다시 들어왔는지 확인한다.

검토 결과는 한국어로, 심각도와 파일ㆍ줄 근거를 포함해 간결히 제시한다. 제안은 자동
반영하지 않으며, Codex가 코드와 테스트로 확인한 최소 변경만 채택한다.
